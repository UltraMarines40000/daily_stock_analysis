...
| Secret 名称 | 说明 | 必填 |
|------------|------|:----:|
| `TOTAL_INVESTMENT_AMOUNT` | 总投资金额，整数，例如 `100000` | ✅ |
| `HOLDING_DETAILS` | 持仓三元组列表，例如 `[["600519",1680.5,100],["000001",12.3,500]]` | ✅ |
| `POPULAR_STOCK_AUTO_ENABLED` | 是否自动抓取东方财富人气榜股票池，默认 `true` | 可选 |
| `POPULAR_STOCK_LIMIT` | 自动股票池数量，默认 `100` | 可选 |
| `STOCK_LIST` | 手动股票列表；仅当 `POPULAR_STOCK_AUTO_ENABLED=false` 或抓取失败时回退使用 | 可选 |

批量任务会先准备整个人气股票池的行情、趋势、筹码和新闻情报，再把全部候选与账户资金/持仓一次性交给 LLM 生成全局买卖委托，避免逐股分析导致预算被前序股票占用。
