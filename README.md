...
| `POPULAR_STOCK_LIMIT` | 自动股票池数量，例如 `5`；为空则任务直接失败 | ✅ |

批量任务会先准备整个人气股票池的行情、趋势、筹码和新闻情报，再把全部候选与账户资金/持仓一次性交给 LLM 生成全局买卖委托，避免逐股分析导致预算被前序股票占用。东方财富人气榜抓取失败时任务会直接失败，不会回退到手动股票列表。
